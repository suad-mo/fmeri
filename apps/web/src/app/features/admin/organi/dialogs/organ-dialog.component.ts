...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ data.organ ? 'Uredi organ' : 'Novi organ uprave' }}</h2>
    ...
    <mat-dialog-content>
      <form [formGroup]="form" class="dialog-form">
        ...
        <mat-form-field appearance="outline">
          <mat-label>Zakonski osnov (jedan po redu)</mat-label>
          <textarea matInput formControlName="nadleznost" rows="2"></textarea>
<textarea matInput formControlName="zakonskiOsnovText" rows="3"
  placeholder="Zakon o ministarstvima FBiH&#10;Zakon o organizaciji organa uprave FBiH"></textarea>
  ...
  spremi() {
    ...
    const data: Partial<Organ> = {
      ...
      zakonskiOsnov:   raw.zakonskiOsnovText
        ? raw.zakonskiOsnovText.split('\n').map(s => s.trim()).filter(Boolean)
        : [],
      uSastavu:        raw.uSastavu ?? false,
      nadredjeniOrgan: raw.nadredjeniOrgan ? { _id: raw.nadredjeniOrgan } as any : undefined,
    };
